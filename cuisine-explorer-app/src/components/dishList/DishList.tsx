...
import "./DishList.css";
import { DIET_TYPE_OPTIONS } from "../../utils/helper";
...
const DishList = () => {
  ...
  const filteredDishes = data.getAllDishes.filter((dish: Dish) => {
    return (
      ...
      (!filters.state ||
        dish.state.toLowerCase() === filters.state.toLowerCase()) &&
      (!filters.region ||
        ...
        <tbody>
          {currentDishes.map((dish: Dish) => (
            <tr key={dish.name}>
              ...
              <td>
                {dish.diet.slice(0, 1).toUpperCase() + dish.diet.slice(1)}
              </td>
              <td>
                {dish.course.slice(0, 1).toUpperCase() + dish.course.slice(1)}
              </td>
